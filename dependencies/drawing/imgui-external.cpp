...
#include "../../cheat/hacks/features.h"
...
namespace drawing
{
    ...
    void  drawing_External::overlay(std::function<void()> CallBack)
    {
        std::string DestWindowName = "Counter-Strike 2";
        std::string DestWindowClassName = "SDL_APP";
        ...
        DestWindow.hWnd = FindWindowA(DestWindowClassName.c_str(), DestWindowName.c_str());
    ...
    bool drawing_External::PeekEndMessage()
    {
        MSG msg;
    ...
    void drawing_External::MainLoop()
    {
        while (!EndFlag)
        {
            if (PeekEndMessage())
    ...
    bool drawing_External::CreateMyWindow()
    {
    ...
    bool drawing_External::UpdateWindowData()
    {
        POINT Point{};
        RECT Rect{};
        ...
        Window.Pos = DestWindow.Pos = Vector2(static_cast<float>(Point.x), static_cast<float>(Point.y));
        Window.Size = DestWindow.Size = Vector2(static_cast<float>(Rect.right), static_cast<float>(Rect.bottom));
