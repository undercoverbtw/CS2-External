...
public:
    ...
    vector < const char* > aimbot_sub_tabs = { "Legitbot  ", "Ragebot"};

    void prepared_popup(const char* id, const char* name, std::function<void()> content);
    bool settings_widget(const char* u_id);

    void colored_rect(const char* str_id, const ImColor color, const ImVector2 size);
    void picker_widget(const char* str_id, const char* content, const ImVector2 size);
    ...
    void group_box(const char* name, ImVector2 size_arg);
    void end_group_box();

    void group_box_alternative(const char* name, ImVector2 size_arg, ImVector2 padding = ImVector2(12, 12));
    ...
    bool button(const char* label, const char* icon, const ImVector2& size_arg = ImVector2(0, 0), ImGuiButtonFlags flags = ImGuiButtonFlags_None);
    bool button_alternative(const char* icon, const ImVector2& size_arg = ImVector2(0, 0));

    void list_box(const char* name, ImVector2 size_arg, std::function< void() > content, ImVector2 padding = ImVector2(8, 8));
    ...
    bool selectable(const char* label, bool* p_selected, ImGuiSelectableFlags flags = 0, const ImVector2& size = ImVector2(0, 0));      // "bool* p_selected" point to the selection state (read-write), as a convenient helper.

    void image(ImTextureID user_texture_id, const ImVector2& size, float rounding, ImDrawFlags flags, const ImVector2& uv0 = ImVector2(0, 0), const ImVector2& uv1 = ImVector2(1, 1), const ImVec4& tint_col = ImVec4(1, 1, 1, 1), const ImVec4& border_col = ImVec4(0, 0, 0, 0));
